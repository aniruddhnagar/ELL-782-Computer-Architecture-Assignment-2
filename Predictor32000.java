public class Predictor32000 extends Predictor {
	...
	//====================================== GSHARE + LOCAL - BRANCH - PREDICTOR =====================================
		...
		// Constructor for Predictor 32000 ===========================================================================	
		public Predictor32000() {
	
			//initializing Select table ------------------------------------------------------------------------------		
			tab_Select = new Table(1<<12,2);
			pc = new Register(32);	
		
			//initializing Gshare-------------------------------------------------------------------------------------
			pht_tab_G = new Table(1<<12,2);
			ghr_reg_G = new Register(12);
			
			//initializing Local -------------------------------------------------------------------------------------	
			lht_tab_LH = new Table(1<<10,11);
			pht_tab_LH = new Table(1<<11,2);
		}
		...
		public boolean L_trainer(long address, boolean outcome, boolean predict){
			...
			int ind_lht_tab_LH = pc.getInteger(21,30);
			int ind_pht_tab_LH = lht_tab_LH.getInteger(ind_lht_tab_LH,0,10);
			...
			lht_tab_LH.setInteger(ind_lht_tab_LH,0,10,ind_pht_tab_LH);
			return LH_predict;
		}
		...
		public boolean G_trainer(long address, boolean outcome, boolean predict){
			...
			pc.setInteger(0,31,(int)address);
			int val_ghr_reg_G = ghr_reg_G.getInteger(0,11);
			int	ind_pht_tab_G = (pc.getInteger(19,30)) ^ val_ghr_reg_G;
			temp_ref = ind_pht_tab_G;
			...
			ghr_reg_G.setInteger(0,11,val_ghr_reg_G);
			return G_predict;
		}
	...
	//==================================================================================================================
		...
		public boolean predict(long address){
			...
			// Gshare --------------------------------------------------------------------------------------------------
			int val_ghr_reg_G = ghr_reg_G.getInteger(0,11);
			int ind_pht_tab_G = pc.getInteger(19,30);
			ind_pht_tab_G = ind_pht_tab_G ^ val_ghr_reg_G;
			int sat_val_G = pht_tab_G.getInteger(ind_pht_tab_G,0,1);
			
			// select table---------------------------------------------------------------------------------------------
			int ind_tab_select = ind_pht_tab_G;
			int val_tab_select = tab_Select.getInteger(ind_tab_select,0,1);	
